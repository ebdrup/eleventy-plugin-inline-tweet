...
const URL_REGEXP = /embedTweet:https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_+.~#?&//=]*)/g;

module.exports = async function embedTweet(eleventyConfig, options) {
  options = Object.assign({ forceReload: false, skipWritingFiles: false }, options);
  eleventyConfig.addTransform('embedYouTube', async (pageHtml, outputPath) => {
    if (!outputPath.endsWith('.html')) {
      return pageHtml;
    }
    const urls = [...pageHtml.matchAll(URL_REGEXP)]
      .map(match => match.splice(0, 1))
      .map(matches => matches.pop().replace('embedTweet:', ''));
    if (urls.lenght === 0) {
      return pageHtml;
    }
    for (const url of urls) {
      pageHtml = pageHtml.replace(`embedTweet:${url}`, await getEmbeddedTweet(url, options));
    }
    return pageHtml;
  });
};

async function getEmbeddedTweet(url, { forceReload, skipWritingFiles }) {
  const file = path.join(process.cwd(), '.embedTweet.json');
  let tweetCache = {};
  try {
